...
class DestinationPersistenceServiceTest {
	...
	@InjectMocks
	private DestinationPersistenceService service;

	@Captor
	ArgumentCaptor<List<Destination>> destinationsCaptor;
	...
	void persistExistingDestinations_AddsMissingProviderMappings() {
		...
		CommonDestination rawDto = new CommonDestination("D123",
	...
	void persistExistingDestinations_SkipsWhenMappingExists() {
		...
		DestinationProviderMapping existingMapping = new DestinationProviderMapping(existingDestination,
				provider,
				"D123");
	...
	void persistNewDestinations_WhenProviderNotFound_ThrowsException() {
		...
		assertThrows(IllegalStateException.class,
				() -> service.persistNewDestinations(providerName, isoToDestinations));
		verifyNoInteractions(destinationRepository);
	}
	...
	void persistNewDestinations_MultipleCountriesWithMixedValidity() {
		...
		Map<String, List<CommonDestination>> input = Map
				.of(validIso, List.of(mockRawDestinationDTO()), invalidIso, List.of(mockRawDestinationDTO()));
	...
	void persistNewDestinations_WithMultipleTranslations_CreatesAll() {
		CommonDestination dto = new CommonDestination("D123",
				List.of(new CommonDestination.Translation(LanguageCode.EN, "Singapore"),
	...
	private static CommonDestination mockRawDestinationDTO() {
		...
		return new CommonDestination("test-id",
