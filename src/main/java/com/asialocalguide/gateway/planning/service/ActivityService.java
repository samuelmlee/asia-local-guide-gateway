...
public class ActivityService {
	...
	public void cacheNewActivitiesByProvider(Map<BookingProviderName, Set<String>> providerNameToId) {
		...
		List<FetchActivityStrategy> strategiesToUse = fetchActivityStrategies.stream()
		...
		if (strategiesToUse.isEmpty()) {
			throw new ActivityCachingException(
					String.format("No strategies found for provider name to ID mapping: %s, strategies in service: %s",
							providerNameToId,
							strategiesToUse));
		}

		Map<BookingProviderName, BookingProvider> nameToProvider = bookingProviderService.getAllBookingProviders()
				.stream()
		...
		Map<LanguageCode, Language> codeToLanguage = languageService.getAllLanguages()
				.stream()
	...
	private void validatePersistNewActivitiesInput(Map<BookingProviderName, Set<String>> providerNameToId,
			List<FetchActivityStrategy> fetchActivityStrategies) {
		if (providerNameToId == null || providerNameToId.isEmpty() || fetchActivityStrategies == null
	...
	private Set<String> buildActivityIdsForCaching(FetchActivityStrategy strategy,
		...
		try {
			...
			if (cachedIds == null || cachedIds.isEmpty()) {
				log.info("No existing activity IDs found for providerName: {} and activityIds:  {}",
						providerName,
						activityIds);
				return activityIds;
			}
			...
			log.warn("Error while building activity IDs for caching for providerName: {}, activityIds: {}",
					providerName,
					activityIds,
					ex);
	...
	private List<Activity> convertToActivities(List<CommonPersistableActivity> persistableActivities,
			BookingProvider bookingProvider, Map<LanguageCode, Language> codeToLanguage) {

		if (persistableActivities == null || persistableActivities.isEmpty() || bookingProvider == null) {
			log.warn("Invalid persistableActivities; {} or BookingProvider: {} in convertToActivities",
	...
	private Optional<Activity> toActivity(CommonPersistableActivity persistable, BookingProvider bookingProvider,
		...
		try {
			...
			Activity activity = new Activity(persistable.providerId(),
					bookingProvider,
			...
			// Set translations
			Map<LanguageCode, String> titlesByLanguage = persistable.title()
					.stream()
					.filter(title -> title.languageCode() != null)
					.collect(Collectors.toMap(CommonPersistableActivity.Translation::languageCode,
							CommonPersistableActivity.Translation::value));

			Map<LanguageCode, String> descriptionsByLanguage = persistable.description()
					.stream()
			...
			// Title is required
			titlesByLanguage.forEach((languageCode, title) -> {
				try {
			...
			Optional.ofNullable(persistable.images())
					.ifPresent(images -> images.forEach(image -> activity
							.addImage(new ActivityImage(image.height(), image.width(), image.url(), image.type()))));

			return Optional.of(activity);

		} catch (Exception e) {
			log.warn("Error while converting a CommonPersistableActivity : {}, for BookingProvider: {} in toActivity",
					persistable,
					bookingProvider,
					e);
