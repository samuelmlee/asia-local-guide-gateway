...
public class DestinationPersistenceService {
	...
	/**
	 * Persists existing Destinations by adding a new DestinationProviderMapping for
	 * each provider.
	 *
	 * @param idToRawDestinations Map<ProviderName, Map<DestinationId,
	 *                            List<RawDestinationDTO>>>
	 */
	@Transactional
	public void persistExistingDestinations(BookingProviderName providerName,
			Map<UUID, CommonDestination> idToRawDestinations) {
		...
		BookingProvider provider = bookingProviderService.getBookingProviderByName(providerName)
		...
		existingDestinations.forEach(destination -> {
			...
			if (destination.getBookingProviderMapping(provider.getId()).isEmpty()) {
				DestinationProviderMapping mapping = new DestinationProviderMapping(destination,
						provider,
	 ...
	 *
	 * @param isoCodeToRawDestinations Map<ProviderName, Map<IsoCode,
	 *                                 List<RawDestinationDTO>>>
	 */
	@Transactional
	public void persistNewDestinations(BookingProviderName providerName,
		...
		if (providerName == null || isoCodeToRawDestinations == null || isoCodeToRawDestinations.isEmpty()) {
			log.warn(
					"Abort processing RawDestinationDTO: BookingProviderName is null or List<RawDestinationDTO>> is empty");
			return;
		}
		...
		Map<String, Country> countryMap = countryService.findByIso2CodeIn(isoCodes)
				.stream()
		...
		for (var isoEntry : isoCodeToRawDestinations.entrySet()) {
			...
			if (country == null) {
				log.warn("Country not found for ISO Code: {}, List<RawDestination> not processed : {}",
						isoCode,
						commonDestinations);
				continue;
			}
			...
			for (CommonDestination rawDto : nonNullDestinations) {
				...
				rawDto.names()
						.forEach(name -> Optional.ofNullable(codeToLanguage.get(name.languageCode()))
								.ifPresentOrElse(
										language -> newDestination.addTranslation(
				...
				DestinationProviderMapping mapping = new DestinationProviderMapping(newDestination,
						provider,
						rawDto.destinationId());
