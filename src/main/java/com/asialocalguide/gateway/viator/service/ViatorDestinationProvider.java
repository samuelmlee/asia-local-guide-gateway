...
public class ViatorDestinationProvider implements DestinationProvider {
    ...
    private CommonDestination createRawDestinationDTO(
    ...
    private List<CommonDestination.Translation> resolveTranslations(
        ...
        return languageToDestinations.entrySet().stream()
                .map(
                        entry -> {
                            ...
                            return new CommonDestination.Translation(entry.getKey().toString(), dto.name());
                        })
                .filter(Objects::nonNull)
                .toList();
    }
